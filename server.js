...
async function seedShop() {
  await Promise.all(titleItems.map((item) => Title.updateOne({ key: item.key }, { $set: item }, { upsert: true })));
  await Promise.all(badgeItems.map((item) => Badge.updateOne({ key: item.key }, { $set: item }, { upsert: true })));
}
...
async function getProfile(guildId, userId) {
  const [levelData, moneyData, attendanceData, inv, titles, badges] = await Promise.all([LevelSystem.findOne({ GuildID: guildId, UserID: userId }).lean(), Money.findOne({ userid: userId }).lean(), Attendance.findOne({ userid: userId }).lean(), Inventory.findOne({ userId }).lean(), Title.find().lean(), Badge.find().lean()]);
  const level = toInt(levelData?.level, 1); const xp = toInt(levelData?.xp, 0); const needed = calculateXP(level);
  const ownedTitleMap = new Set(inv?.titles || []); const ownedBadgeMap = new Set(inv?.badges || []);
  return { guildId, userId, level, xp, needed, progress: needed ? Math.min(100, Math.round((xp / needed) * 1000) / 10) : 0, money: toInt(moneyData?.money, 0), attendance: toInt(attendanceData?.count, 0), activeTitle: inv?.activeTitle || '', ownedTitles: titles.filter((t) => ownedTitleMap.has(t.key)), ownedBadges: badges.filter((b) => ownedBadgeMap.has(b.key)) };
}

app.get('/api/config', (req, res) => res.json({ siteTitle: process.env.SITE_TITLE || 'NATSUMI Game', defaultGuildId: DEFAULT_GUILD_ID, donationEnabled: Boolean(DONATION_URL), donationUrl: DONATION_URL }));
...
app.post('/api/buy', async (req, res) => {
  ...
  const Item = itemType === 'title' ? Title : Badge; const item = await Item.findOne({ key }).lean(); if (!item) return res.status(404).json({ error: '상점 아이템을 찾지 못했어.' });
  const moneyData = await ensureMoney(userId); const currentMoney = toInt(moneyData.money, 0); if (currentMoney < item.price) return res.status(400).json({ error: `금전 부족! 필요 ${item.price}, 보유 ${currentMoney}` });
  const field = itemType === 'title' ? 'titles' : 'badges'; const inv = await Inventory.findOne({ userId }); if (inv?.[field]?.includes(key)) return res.status(400).json({ error: '이미 가지고 있는 아이템이야.' });
  await addMoney(userId, -item.price); await Inventory.updateOne({ userId }, { $addToSet: { [field]: key }, ...(itemType === 'title' ? { $setOnInsert: { activeTitle: key } } : {}) }, { upsert: true });
  res.json({ ok: true, message: `${item.emoji} ${item.name} 구매 완료!` });
});

app.post('/api/title/select', async (req, res) => { const { userId, key } = req.body; const inv = await Inventory.findOne({ userId }); if (!inv?.titles?.includes(key)) return res.status(400).json({ error: '보유하지 않은 칭호야.' }); await Inventory.updateOne({ userId }, { $set: { activeTitle: key } }); res.json({ ok: true }); });

app.post('/api/games/slot', async (req, res) => {
  const { userId } = req.body; const bet = clampBet(req.body.bet); if (!userId) return res.status(400).json({ error: 'userId가 필요해.' });
  const money = await ensureMoney(userId); if (toInt(money.money, 0) < bet) return res.status(400).json({ error: '금전 부족!' });
  const emojis = ['🍒','🍋','🍇','🔔','💎','7️⃣','🍀','🍎','🍐','🍊']; const reels = [pick(emojis), pick(emojis), pick(emojis)];
  let delta = -bet, result = '꽝'; if (reels[0] === reels[1] && reels[1] === reels[2]) { delta = reels[0] === '7️⃣' ? bet * 20 : bet * 10; result = reels[0] === '7️⃣' ? '슈퍼 잭팟' : '잭팟'; } else if (new Set(reels).size === 2) { delta = Math.floor(bet * 1.5); result = '당첨'; }
  const after = await addMoney(userId, delta); res.json({ game: 'slot', reels, result, delta, money: toInt(after.money, 0) });
});

app.post('/api/games/fishbun', async (req, res) => {
  const { userId } = req.body; const bet = clampBet(req.body.bet); if (!userId) return res.status(400).json({ error: 'userId가 필요해.' });
  const money = await ensureMoney(userId); if (toInt(money.money, 0) < bet) return res.status(400).json({ error: '금전 부족!' });
  const chance = Math.random() * 100; let item = '🔥 시꺼멓게 탄 붕어빵', delta = -bet, rarity = 'fail';
  if (chance <= 0.5) { item = '✨ 전설의 황금 잉어빵'; delta = bet * (10 + Math.floor(Math.random() * 10)); rarity = 'legend'; }
  else if (chance <= 15.5) { item = '🍯 슈크림 대왕 붕어빵'; delta = Math.floor(bet * (1.5 + Math.random() * 4.5)); rarity = 'rare'; }
  else if (chance <= 40.5) { item = '🐟 클래식 팥 붕어빵'; delta = Math.floor(bet * (0.9 + Math.random() * 0.9)); rarity = 'normal'; }
  const after = await addMoney(userId, delta); res.json({ game: 'fishbun', item, rarity, delta, money: toInt(after.money, 0) });
});

app.post('/api/games/mine', async (req, res) => {
  const { userId } = req.body; const bet = clampBet(req.body.bet); if (!userId) return res.status(400).json({ error: 'userId가 필요해.' });
  const money = await ensureMoney(userId); if (toInt(money.money, 0) < bet) return res.status(400).json({ error: '금전 부족!' });
  const roll = Math.random(); let item = '🪨 평범한 돌멩이', delta = -bet;
  if (roll < 0.03) { item = '💎 전설의 다이아 광맥'; delta = bet * 12; } else if (roll < 0.18) { item = '🥇 황금 광맥'; delta = bet * 3; } else if (roll < 0.48) { item = '🪙 은빛 광석'; delta = Math.floor(bet * 1.2); }
  const after = await addMoney(userId, delta); res.json({ game: 'mine', item, delta, money: toInt(after.money, 0) });
});

app.post('/api/games/mole', async (req, res) => {
  const { userId, difficulty = 'easy', score = 0 } = req.body; if (!userId) return res.status(400).json({ error: 'userId가 필요해.' });
  const configs = { safety: 25, easy: 100, medium: 400, hard: 1600 }; const safeScore = Math.max(0, Math.min(500, toInt(score, 0))); const reward = safeScore * (configs[difficulty] || configs.easy);
  const after = await addMoney(userId, reward); res.json({ game: 'mole', difficulty, score: safeScore, delta: reward, money: toInt(after.money, 0) });
});

app.post('/api/games/fishing', async (req, res) => {
  const { userId } = req.body; if (!userId) return res.status(400).json({ error: 'userId가 필요해.' });
  await ensureMoney(userId); let inv = await FishingInventory.findOne({ userId }); if (!inv) inv = await FishingInventory.create({ userId });
  const chance = Math.random() * 100; let item = '🪵 떠내려온 나뭇가지', field = '', reward = 50;
  if (chance <= 0.5) { item = '✨ 황금 물고기'; field = 'goldenFish'; reward = 100000; } else if (chance <= 1.5) { item = '💫 괜찮은 황금 물고기'; field = 'decentGoldenFish'; reward = 20000; } else if (chance <= 11.5) { item = '🐟 중형 물고기'; field = 'mediumFish'; reward = 3000; } else if (chance <= 31.5) { item = '🐠 일반 물고기'; field = 'regularFish'; reward = 800; } else if (chance <= 61.5) { item = '🧩 수상한 잡동사니'; field = 'curiousItem'; reward = 300; }
  if (field) { inv[field] += 1; await inv.save(); }
  const after = await addMoney(userId, reward); res.json({ game: 'fishing', item, delta: reward, inventory: inv, money: toInt(after.money, 0) });
});

app.get('/rank-card/:guildId/:userId', async (req, res) => {
  const p = await getProfile(req.params.guildId, req.params.userId); const active = p.ownedTitles.find((t) => t.key === p.activeTitle); const badges = p.ownedBadges.slice(0, 8).map((b) => `${b.emoji} ${b.name}`).join(' · ');
